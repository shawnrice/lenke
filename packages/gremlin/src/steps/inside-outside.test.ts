...
describe('between / inside / outside predicates', () => {
  ...
  test('between is half-open [first, second) — Gremlin semantics', () => {
    ...
    const r = arr(
      run(traversal(V(), hasLabel('PERSON'), has('age', between(29, 32)), values('name')), tinkerGraph),
    );
  ...
  test('inside is strict open (first, second)', () => {
    ...
    const r = arr(
      run(traversal(V(), hasLabel('PERSON'), has('age', inside(27, 35)), values('name')), tinkerGraph),
    );
  ...
  test('outside is strict complement: < first || > second', () => {
    ...
    const r = arr(
      run(traversal(V(), hasLabel('PERSON'), has('age', outside(29, 32)), values('name')), tinkerGraph),
    );
