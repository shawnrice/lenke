...
describe('Repeat / iterative reachability', () => {
  ...
  test('repeat(out()).times(2) from marko reaches grandchildren', () => {
    const r = arr(
      run(traversal(V('1'), repeat(out()).times(2), values('name')), g),
    );
  ...
  test('repeat(out()).until(hasLabel(SOFTWARE)) yields software-terminal paths', () => {
    const r = arr(
      run(
        traversal(V('1'), repeat(out()).until(hasLabel('SOFTWARE')), path().by('name')),
        g,
      ),
    );
  ...
  test('emit only software (has lang)', () => {
    const r = arr(
      run(
        traversal(
          V('1'),
          repeat(out()).times(2).emit(has('lang', eq('java'))),
          values('name'),
        ),
        g,
      ),
  ...
  test('repeat(out()).times(1) is the same as out() from marko', () => {
    const r = arr(
      run(traversal(V('1'), repeat(out()).times(1), values('name')), g),
    );
