...
const parseTime = (s: string): [number, number] => {
  ...
  if (parts.length !== 3) {throw new Error(`bad time '${s}'`);}
...
export function parseDate(s: string): LocalDate {
  ...
  if (m < 1 || m > 12 || d < 1 || d > 31) {throw new Error(`date out of range '${s}'`);}
...
export function parseDateTime(s: string): LocalDateTime {
  ...
  if (sep === -1) {throw new Error(`datetime missing time part '${s}'`);}
...
export function parseDuration(s: string): Duration {
  if (!s.startsWith('P')) {throw new Error(`duration must start with 'P': '${s}'`);}
  ...
  const takeNum = (designator: string): number => {
    if (num === '') {throw new Error(`missing number before '${designator}' in '${s}'`);}
  ...
  for (const c of datePart) {
    if (/[0-9-]/.test(c)) {num += c;}
    else if (c === 'Y') {months += takeNum('Y') * 12;}
    else if (c === 'M') {months += takeNum('M');}
    else if (c === 'W') {days += takeNum('W') * 7;}
    else if (c === 'D') {days += takeNum('D');}
    else {throw new Error(`bad duration date field '${c}' in '${s}'`);}
  }

  if (num !== '') {throw new Error(`dangling number in duration '${s}'`);}
  ...
  if (timePart !== undefined) {
    for (const c of timePart) {
      if (/[0-9.-]/.test(c)) {num += c;}
      else if (c === 'H') {secs += takeNum('H') * 3600;}
      else if (c === 'M') {secs += takeNum('M') * 60;}
      else if (c === 'S') {
        const dot = num.indexOf('.');
        const whole = dot === -1 ? num : num.slice(0, dot);
        const frac = dot === -1 ? undefined : num.slice(dot + 1);

        if (whole === '') {throw new Error(`missing number before 'S' in '${s}'`);}

        secs += parseIntStrict(whole);
        nanos = parseFrac(frac);
        num = '';
      } else {throw new Error(`bad duration time field '${c}' in '${s}'`);}
    }
...
export function formatDuration(d: Duration): string {
  ...
  if (d.months !== 0) {out += `${d.months}M`;}

  if (d.days !== 0) {out += `${d.days}D`;}

  if (d.secs !== 0 || d.nanos !== 0) {out += `T${d.secs}${fmtFrac(d.nanos)}S`;}
...
export function temporalFormat(t: Temporal): string {
  if (t instanceof LocalDate) {return formatDate(t);}

  if (t instanceof LocalDateTime) {return formatDateTime(t);}
...
export function temporalParse(tag: string, s: string): Temporal {
  if (tag === 'date') {return parseDate(s);}

  if (tag === 'datetime') {return parseDateTime(s);}

  if (tag === 'duration') {return parseDuration(s);}
...
export function graphsonType(t: Temporal): string {
  if (t instanceof LocalDate) {return 'gx:LocalDate';}

  if (t instanceof LocalDateTime) {return 'gx:LocalDateTime';}
...
export function graphsonTag(ty: string): 'date' | 'datetime' | 'duration' | undefined {
  if (ty === 'gx:LocalDate') {return 'date';}

  if (ty === 'gx:LocalDateTime') {return 'datetime';}

  if (ty === 'gx:Duration') {return 'duration';}
...
export function fromTaggedJson(v: unknown): Temporal | null {
  ...
  if (key !== '@date' && key !== '@datetime' && key !== '@duration') {return null;}
...
const kindRank = (t: Temporal): number => {
  if (t instanceof LocalDate) {return 0;}

  if (t instanceof LocalDateTime) {return 1;}
...
export function temporalCmpTotal(a: Temporal, b: Temporal): number {
  if (a instanceof LocalDate && b instanceof LocalDate) {return cmpNum(a.days, b.days);}
