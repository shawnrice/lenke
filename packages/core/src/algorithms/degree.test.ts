...
describe('degree centrality', () => {
  test('out-degree (default) over all edge types', async () => {
    expect(map(await degree({}, modern()))).toEqual({ 1: 3, 2: 0, 3: 0, 4: 2, 5: 0, 6: 1 });
  });

  test('in-degree over all edge types', async () => {
    expect(map(await degree({ direction: 'in' }, modern()))).toEqual({
      1: 0,
      2: 1,
      3: 3,
      4: 1,
  ...
  test('both directions — self-loop-free graph sums out + in', async () => {
    expect(map(await degree({ direction: 'both' }, modern()))).toEqual({
      1: 3,
      2: 1,
      3: 3,
      4: 3,
  ...
  test('typed: out KNOWS only marko', async () => {
    expect(map(await degree({ direction: 'out', edgeLabel: 'KNOWS' }, modern()))).toEqual({
      1: 2,
      2: 0,
      3: 0,
      4: 0,
  ...
  test('unknown edge type → every vertex zero', async () => {
    expect(
      Object.values(map(await degree({ edgeLabel: 'NOPE' }, modern()))).every((d) => d === 0),
    ).toBe(true);
  });

  test('rows are in insertion order', async () => {
    expect((await degree({}, modern())).map((r) => r.node)).toEqual(['1', '2', '3', '4', '5', '6']);
  });

  test('writeProperty writes each degree back to the vertex', async () => {
    const g = modern();
    await degree({ direction: 'both', writeProperty: 'deg' }, g);
  ...
  test('dual-form: curried application equals direct', async () => {
    ...
    expect(await out(g)).toEqual(await degree({ direction: 'in' }, modern()));
  });
});
