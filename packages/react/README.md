...
> React bindings that turn a `@lenke` labeled-property graph into a reactive data store for React components.
...
- `GraphProvider({ graph, children })` — supplies a `@lenke/core` `Graph` to the hooks below.
...
These hooks drive React from a `@lenke/native` store (`createStore(graph)`) instead of the in-process `Graph`. They import no engine code, so a store-only consumer tree-shakes the TypeScript core/gremlin engines away.
