...
- `graph.createIndex({ on, kind, keys })` — one entry point, four kinds. `kind: 'hash'` (`{ on: 'vertex', kind: 'hash', keys: ['id'] }`) makes a point lookup (`WHERE v.k = $x` / inline `{k: $x}`) a seek instead of a scan; `kind: 'range'` does the same for the inequalities (`WHERE v.k > $x`, `BETWEEN`). Both cover any value type incl. temporals, and both are vertex-only and independent — a key queried both ways wants both declared, and `dropVertexIndex(key)` drops whichever exist. Keys can be dotted for a nested field (`keys: ['meta.errorId']`). `kind: 'interval'` is an edge-only `[lo, hi)` containment/overlap index (lo inclusive, hi exclusive) for bitemporal/valid-time, reservations, or numeric ranges; `kind: 'type'` is the keyless edge-type index, which pays off expanding one edge type off high-degree, many-type nodes. `vertexIndexes()` / `dropVertexIndex(key)` round it out. (Host-API only — there is no GQL `CREATE INDEX`.)
