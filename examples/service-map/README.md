...
- **Gremlin rides the wire (one-shot)** — a `query` with `lang: 'gremlin'` runs the text through the Gremlin engine and answers with `values`; `client.gremlin(traversal)` is its client shape (proven round-tripping the Node server). The blast radius stays native **GQL** var-length (`-[:CALLS]->{1,}` + `DISTINCT`) because GQL covers it, but imperative traversals now cross the same protocol. Caveat: Gremlin has **no param binding** — never splice untrusted input into a traversal (use GQL + `params` for user values). Still open: **live** (epoch-gated) Gremlin subscriptions — the store's `liveQuery` is GQL-only, so those need dep-inference over a traversal plus a values-shaped snapshot.
