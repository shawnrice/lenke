...
//! (`/conformance/codec-corpus.json`) that the TS `@pl-graph/serialization`
...
fn canonical_round_trips_every_format_structurally() {
    let c = corpus();
    let canonical = c["canonical"].as_str().unwrap();
    ...
    assert_eq!(
        want,
        c["canonical_normal"].as_str().unwrap(),
...
fn malformed_inputs_rejected_with_expected_code() {
    let c = corpus();
    for case in c["reject"].as_array().unwrap() {
