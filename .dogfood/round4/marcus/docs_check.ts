...
// native.md: `using g = graphFromNdjson(...)` disposal
{
  ...
  check('native.md param query', () => g.query('MATCH (p:Person) WHERE p.age > $min RETURN p.name', { min: 30 }));
  ...
  check('gql README: AS count should FAIL', () => g.query`MATCH (p:Person) RETURN count(*) AS count`);
  check('gql README: AS `count` works', () => g.query('MATCH (p:Person) RETURN count(*) AS `count`'));
