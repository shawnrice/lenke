...
function schemaV1(): Schema {
  return new Schema()
    ...
    .relationship({ label: 'AUTHORED', from: 'User', to: 'Post', inbound: 'one', cascadeDelete: true })
...
const post1 = repo.createWithOwner({ label: 'Post', props: { title: 'Hello', body: 'first' }, edgeLabel: 'AUTHORED', owner: alice });
line(`  created Org=${acme.getProperty('name')}, Users=[Alice,Bob], Post="${post1.getProperty('title')}" by Alice`);
line(`  (role defaulted to '${alice.getProperty('role')}', plan defaulted to '${acme.getProperty('plan')}')`);

// ---------------------------------------------------------------------------
// 2. Constraint violations are REJECTED at write time (via event veto)
// ---------------------------------------------------------------------------
hr('2. CONSTRAINT VIOLATIONS (rejected)');
expectReject('duplicate unique email', () => repo.create('User', { email: 'alice@x.io', name: 'Al2' }));
expectReject('missing required Post.title', () => repo.createWithOwner({ label: 'Post', props: { body: 'no title' }, edgeLabel: 'AUTHORED', owner: bob }));
expectReject('wrong type (age is string)', () => repo.create('User', { email: 'c@x.io', name: 'C', age: '30' as unknown as number }));
expectReject('unknown property not in schema', () => repo.create('User', { email: 'd@x.io', name: 'D', nickname: 'Dee' } as never));
expectReject('second author on a post (cardinality inbound=one)', () => repo.link(bob, 'AUTHORED', post1));
expectReject('setting a duplicate email via update', () => repo.set(bob, 'email', 'alice@x.io'));
expectReject('removing a required property', () => { bob.removeProperty('name'); const vs = schema.drain(); if (vs.length) throw new ConstraintError(vs); });
...
repoSetup.createWithOwner({ label: 'Post', props: { title: 'Bobs note', body: 'hi' }, edgeLabel: 'AUTHORED', owner: bob });
schemaSetup.detach();
...
const migrations: Migration[] = [
  {
    ...
    up({ byLabel }) {
      ...
      for (const p of byLabel('Post')) {
        if (!p.hasProperty('status')) { p.setProperty('status', 'published'); n++; }
      }
  ...
  },
  {
    ...
    up({ byLabel }) {
      ...
      for (const o of orgs) { o.addLabel('Organization'); o.removeLabel('Org'); }
...
line(`  sample post now: ${JSON.stringify(serialize(graph, 'pg-json').match(/"status":"[^"]+"/)?.[0])}`);
...
const carol = repo2.create('User', { email: 'carol@x.io', name: 'Carol', username: 'carol' } as never);
...
const posts = from(graph, schemaV2, 'Post').where('status', '=', 'published').return('title', 'status');
...
line(`  Users now: ${JSON.stringify([...graph.getVerticesByLabel('User')].map((u) => u.getProperty('username')))}`);
