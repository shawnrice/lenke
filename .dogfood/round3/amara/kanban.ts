...
function buildClient(opts: { seedNdjson?: Uint8Array; initialWrites?: readonly any[]; complete?: readonly string[] }): ClientRig {
  ...
  const serverView = reconnecting.liveQuery(
    'MATCH (c:Card) RETURN c.id AS id, c.col AS col',
    { deps: ['Card', 'id', 'col'] },
  );
...
async function main() {
  ...
  console.log('server view :', fmt(rig.serverView.getSnapshot().rows as any), 'complete=', rig.serverView.getSnapshot().complete);
  ...
  console.log('local view  :', fmt(rig.localView.getSnapshot().rows as any), 'complete=', rig.localView.getSnapshot().complete);
  ...
  rig.engine.mutate("INSERT (:Card {id: $id, title: $t, col: $c})", { id: 'c', t: 'Ship it', c: 'todo' });
  console.log('after local mutate -> local:', fmt(rig.localView.getSnapshot().rows as any), 'pendingWrites=', rig.engine.pendingWrites());
  await waitUntil(() => rig.engine.pendingWrites() === 0, 'write drained upstream');
  await waitUntil(() => (rig.serverView.getSnapshot().rows as any).length === 3, 'server view sees c');
  ...
  serverStore.mutate((g) => g.query("MATCH (c:Card {id: $id}) SET c.col = $col", { id: 'a', col: 'doing' }));
  ...
  await waitUntil(() => (rig.serverView.getSnapshot().rows as any).some((r: any) => r.id === 'a' && r.col === 'doing'), 'replay delivered a:doing');
  console.log('server view after replay:', fmt(rig.serverView.getSnapshot().rows as any), '(picked up a:doing without re-subscribing by hand)');
  ...
  rig.engine.mutate("INSERT (:Card {id: $id, title: $t, col: $c})", { id: 'd', t: 'Offline task', c: 'todo' });
  ...
  console.log('while offline -> pendingWrites=', rig.engine.pendingWrites(), '| server truth still:', serverTruth());
  const stranded = rig.engine.queuedWrites();
  console.log('queued (stranded) writes:', stranded.map((w) => w.text.slice(0, 24) + '…'));
  ...
  const snapStore = createSnapshotStore({ filename: 'kanban.snapshot', key, durable: fileStorage(SNAP_PATH) });
  ...
  console.log('wrote', onDisk.length, 'bytes; encrypted flag byte =', onDisk[/* magic4 + u32 len + header */ 0] === 0x4c ? 'LNKS magic ok' : 'BAD');
  // Prove it is NOT plaintext: the card titles must not appear in the bytes.
  const asText = new TextDecoder('latin1').decode(onDisk);
  console.log('plaintext leak check — "Offline task" present in file bytes?', asText.includes('Offline task'));
  ...
  const frozen: SnapshotStorage = { read: async () => onDisk, write: async () => {}, delete: async () => {} };
  const wrongKey = await importSnapshotKey(crypto.getRandomValues(new Uint8Array(32)));
  const wrongStore = createSnapshotStore({ filename: 'kanban.snapshot', key: wrongKey, durable: frozen });
  console.log('load with WRONG key ->', await wrongStore.load({ schemaVersion: 'v1', userId: 'amara' }), '(null = cold boot, correct)');
  ...
  console.log('decrypted snapshot: ndjson', snap.ndjson.length, 'bytes, pendingWrites=', snap.pendingWrites.length, ', collections=', snap.header.collections);
  ...
  console.log('warm local view (from disk, no server needed):', fmt(rig.localView.getSnapshot().rows as any));
  ...
  console.log('server truth after warm-boot drain:', serverTruth(), '(d + b:doing resumed replication)');
